...
| `npx tsx tools/osm/probe-pois.mts` | นับประเภทสถานที่ (POI) จริงจาก OpenFreeMap รอบ 8 เมืองทั่วโลก — ใช้ตัดสินใจว่าจะเพิ่มหมุดประเภทไหน |
...
| **วางหมุดบนแผนที่ (แอดมิน)** | ⚙️ ตั้งค่า → โหมดแอดมิน → ใส่รหัส → ปุ่ม **PIN** ขวามือ (ต้องรัน `dev:all` หรือ `dev:lan`) |

**รหัสแอดมิน**: ตั้งผ่าน env `ADMIN_KEY` ตอนรันเซิร์ฟเวอร์ ถ้าไม่ตั้ง เซิร์ฟเวอร์จะสุ่มให้ครั้งแรกแล้วเก็บไว้ที่ `apps/server/data/admin-key.txt`
และพิมพ์ในบรรทัด `Map pins: … · admin key: …` ตอนเปิด · หมุดเก็บที่ `apps/server/data/pins.json` (ทั้งโฟลเดอร์ `data/` ไม่อยู่ใน git —
แต่ละเครื่องที่รันเซิร์ฟเวอร์มีหมุดของตัวเอง) · ย้ายไฟล์ได้ด้วย env `PINS_FILE`
